...
def check_case_and_duplicates(cfg: dict) -> dict:
...
def normalize_case_and_dups(cfg: dict):
    ...
    def normalize(seq):
        seen = set(); out = []; removed = 0; cased = False
        for s in seq:
            norm = s.strip().lower()
            if norm != s:
                cased = True
    ...
    if removed or cased or elist != fixed:
        ...
        if removed: changes.append(f"EMAIL_LIST (removed {removed} duplicates)")
        elif cased: changes.append("EMAIL_LIST (fixed case)")
    ...
    for label, configurations in (updated.get("SENDER_TO_LABELS") or {}).items():
        for i, group in enumerate(configurations or []):
            ...
            if removed or cased or emails != fixed:
                ...
                if removed: changes.append(f"{loc} (removed {removed} duplicates)")
                elif cased: changes.append(f"{loc} (fixed case)")
