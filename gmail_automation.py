...
def check_files_existence():
    ...
    if not last_run_file_exists:
        logging.debug(
            f"Last run file: '{LAST_RUN_FILE}' does not exist. Will use default time."
        )
    else:
    ...
    for retry in range(max_retries):
        try:
            ...
            if error.resp.status in [429, 403]:  # 429: Rate limit, 403: Forbidden
                wait_time = min(
                    (2**retry) + random.uniform(0, 1), 64
                )  # Cap wait time to a max of 46 secs
                logging.warning(
...
def get_message_details(service, userId, msg_id):
    ...
    try:
        message = execute_request_with_backoff(
            service.users().messages().get(userId=userId, id=msg_id)
        )
        if (
            not message
...
def get_message_details_cached(service, userId, msg_id):
    ...
    if msg_id in message_details_cache:
        ...
        if isinstance(cached_value, tuple) and len(cached_value) == 4:
            ...
            logging.warning(
                f"Invalid cache format for message ID {msg_id}: {cached_value}"
            )
...
def batch_fetch_messages(service, userId, msg_ids):
    ...
    for msg_id in msg_ids:
        if msg_id in message_details_cache:
            ...
            try:
                message = (
                    service.users().messages().get(userId=userId, id=msg_id).execute()
                )
    ...
    try:
        ...
        while "nextPageToken" in response:
            ...
            response = (
                service.users()
                .messages()
                .list(userId=userId, q=query, pageToken=page_token)
                .execute()
            )
...
def modify_message(service, userId, msg_id, label_ids, remove_ids, mark_read):
    ...
    try:
        message = execute_request_with_backoff(
            service.users()
            .messages()
            .modify(userId=userId, id=msg_id, body=modify_body)
        )
        if mark_read:
            execute_request_with_backoff(
                service.users()
                .messages()
                .modify(userId=userId, id=msg_id, body={"removeLabelIds": ["UNREAD"]})
            )
        return message
    except HttpError as error:
...
def process_email(
    ...
    logging.debug(
        f"Processing message ID: {msg_id} with subject: {subject}, date: {date}, sender: {sender}"
    )

    # Fetch details using caching
    subject, date, sender, is_unread = get_message_details_cached(
        service, userId, msg_id
    )
...
def process_emails_for_labeling(
    service, userId, existing_labels, config, last_run_time
):
    script_dir = os.path.dirname(os.path.abspath(__file__))
...
def reprocess_unlabeled_emails(
    service, userId, existing_labels, config, processed_email_ids
):
    ...
    for msg_id in processed_email_ids:
        ...
        for keyword, label_info in config["KEYWORDS_TO_LABELS"].items():
            label, mark_read = label_info
            if (
                subject.lower().find(keyword.lower()) != -1
                and existing_labels[label] not in labels_applied
            ):
        ...
        for sender_category, sender_info in config["SENDER_TO_LABELS"].items():
            for info in sender_info:
                if (
                    sender in info["emails"]
                    and existing_labels[sender_category] not in labels_applied
                ):
                    emails_to_reprocess.add(msg_id)

        logging.debug(
            f"Found {len(emails_to_reprocess)} emails that need to be reprocessed."
        )
